...
export class ProductsComponent implements OnInit,OnChanges,OnDestroy {
 ...
 private subscriptionlist:Subscription[]=[];
  ...
  cartarray:Icart[]=[];
  cart:Icart={} as Icart;
  displayedColumns: string[] = ['id', 'name', 'quantity', 'price','img','actions1'];
  ...
  Edit(prodid:number)
  {
   ...
   this.router.navigate(['/Products/EditProduct/',prodid]);
  ...
  Delete(item:Iproduct|undefined)
  {
   ...
   if(confirm("Are you sure to delete "+item?.name)) {
     this.proapiservice.DeleteProduct(proid).subscribe(pro=>{this.router.navigate(['/Products']),location.reload()});
   }
